...
static const uint8_t CMD_MYMS56XX_RESET = 0x1E;
static const uint8_t CMD_MYMS56XX_READ_ADC = 0x00;

/* PROM start address */
static const uint8_t CMD_MYMS56XX_PROM = 0xA0;
...
/*
  constructor
 */
AP_Baro_MYMS5611::AP_Baro_MYMS5611(AP_Baro &baro, AP_HAL::OwnPtr<AP_HAL::Device> dev, enum MYMS56XX_TYPE ms56xx_type)
...
AP_Baro_Backend *AP_Baro_MYMS5611::probe(AP_Baro &baro,
                                       AP_HAL::OwnPtr<AP_HAL::Device> dev,
                                       enum MYMS56XX_TYPE ms56xx_type)
{
    if (!dev)
    {
        return nullptr;
    }
    AP_Baro_MYMS5611 *sensor = new AP_Baro_MYMS5611(baro, std::move(dev), ms56xx_type);
...
bool AP_Baro_MYMS5611::_init()
{
    ...
    _dev->transfer(&CMD_MYMS56XX_RESET, 1, nullptr, 0);
    hal.scheduler->delay(4);

    const char *name = "MYMS5611";
    switch (_ms56xx_type)
    {
    ...
    _dev->register_periodic_callback(10 * AP_USEC_PER_MSEC,
                                     FUNCTOR_BIND_MEMBER(&AP_Baro_MYMS5611::_timer, void));
    return true;
}
 ...
 */
uint16_t AP_Baro_MYMS5611::crc4(uint16_t *data)
{
    uint16_t n_rem = 0;
    uint8_t n_bit;
...
uint16_t AP_Baro_MYMS5611::_read_prom_word(uint8_t word)
{
    const uint8_t reg = CMD_MYMS56XX_PROM + (word << 1);
    uint8_t val[2];
...
uint32_t AP_Baro_MYMS5611::_read_adc()
{
    uint8_t val[3];
    if (!_dev->transfer(&CMD_MYMS56XX_READ_ADC, 1, val, sizeof(val))) {
        return 0;
    }
    return (val[0] << 16) | (val[1] << 8) | val[2];
}

bool AP_Baro_MYMS5611::_read_prom_5611(uint16_t prom[8])
{
    /*
 ...
 * pressure.
*/
void AP_Baro_MYMS5611::_timer(void)
{
    uint8_t next_cmd;
    uint8_t next_state;
...
void AP_Baro_MYMS5611::_update_and_wrap_accumulator(uint32_t *accum, uint32_t val,
...
void AP_Baro_MYMS5611::_calculate_5611()
{
    float dT;
    float TEMP;
    float OFF;
